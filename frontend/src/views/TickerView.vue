  ...
  <div class="ticker-view">
    <div class="app-container">
      ...
      <div class="ticker-nav">
        ...
        >
          {{ $t('ticker.navigation.overview') }}
        </router-link>
        <router-link
          :to="{ name: 'financials', params: { symbol } }"
          class="ticker-nav__link"
        >
          {{ $t('ticker.navigation.financials') }}
        </router-link>
        <router-link
          :to="{ name: 'options', params: { symbol } }"
          class="ticker-nav__link"
        >
          {{ $t('ticker.navigation.options') }}
        </router-link>
        <router-link
          :to="{ name: 'news', params: { symbol } }"
          class="ticker-nav__link"
        >
          {{ $t('ticker.navigation.news') }}
        </router-link>
      </div>
      ...
      <div class="loading-container" v-if="loading">
        ...
        <p>{{ $t('ticker.loading', { symbol: symbol }) }}</p>
      </div>
      ...
      <div class="ticker-content" v-if="!loading && !hasError && tickerInfo">
        ...
        <div class="info-card full-width" v-if="tickerInfo?.data?.longBusinessSummary">
          <h3 class="info-card__title">{{ $t('ticker.about', { companyName: companyName }) }}</h3>
          <div class="info-card__content">
...
export default {
  ...
  setup(props) {
    ...
    const updateChartPeriod = (period) => {
      ...
      fetchHistoryTimeout = setTimeout(async () => {
        try {
          ...
          try {
            await store.dispatch('ticker/fetchHistoryData', period);
            console.log('History data fetched successfully, data points:',
              store.getters['ticker/historyData']?.length || 0);

            // Log a sample of the data points to verify data format
            if (store.getters['ticker/historyData']?.length > 0) {
              console.log('Sample data point:', store.getters['ticker/historyData'][0]);
            }
          } catch (error) {
